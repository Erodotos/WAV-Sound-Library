...
/** @file speedup.c
 *  @brief This is the source file that is responsible for speeding up 
...
int speedup(char *filename){
   ...
   int newSize = last/2;
   byte *newData = (byte*)malloc(sizeof(byte)*newSize);
   

   for(i = 0, j =0; i< last-16; i+=8 , j +=4){
   ...
   AF->subChunk2Size = newSize;
   AF->chunkSize = newSize + 56;
   byte *p = AF->audioData;
   free (p);
